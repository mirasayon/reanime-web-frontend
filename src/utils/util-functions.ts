...
export function getAnimePosterUrlByShikimoriId(shikimori_id: number): string {
    return envClient.resourceServiceUrl + `/get-anime-poster/${shikimori_id}`;
}
export function getCarouselAnimePosterUrl(segment: string) {
    return `/_assets/_animes/_top-chart-anime-posters/${segment}`;
}
