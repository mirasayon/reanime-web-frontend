export function Card({ children, className = "" }: { children: React.ReactNode; className?: string }) {
    return (
        <div className={`bg-white text-white dark:bg-blue-500/20 m-2 rounded-sm shadow p-4 ${className}`}>
            {children}
        </div>
    );
}
...
export function JustAvatarCircleComponent({
    avatarUrl,
    altTitle,
    className = "",
}: {
    ...
    return <img src={avatarUrl} alt={altTitle} className={`${className} w-10 h-10 rounded`} />;
}
