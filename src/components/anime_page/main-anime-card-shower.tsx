...
export function UIAnimeCardMain({ data }: Props) {
    ...
    return (
        ...
        >
            <AnimePosterImageForMainCard
                anime_title={`Обложка от аниме ${data.title}`}
                imgSrc={imgUrl}
            />
            ...
            <div className={"w-40 flex-col flex m-1 p-1 text-sm"}>
                ...
                <ShowRatingShikimoriDataSmartX
                    rating={data.material_data?.shikimori_rating || null}
                />
                ...
                <div className="     p-1 font-bold">
                    {data.material_data?.anime_status === "released"
                        ? "Завершён"
                        : "Онгоинг"}
                </div>
            </div>
