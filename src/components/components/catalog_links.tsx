...
import type { i_describe_genres } from "#T/userinserface";
...
const typed_description_genres: Omit<i_describe_genres, "description">[] = [
    {
...
const genres = typed_description_genres.map((gen) => {
    return {
        title: gen.russian_name,
        link_url: `/genres/${gen.english_name.toLowerCase()}`,
    };
});
export function Filter_animes_links({ is_dark }: { is_dark: boolean }) {
