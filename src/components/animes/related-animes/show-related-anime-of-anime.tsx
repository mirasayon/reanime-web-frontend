...
import { ShowImageForRelatedAnimeSection, ShowRelationTypeForBothMangaAndAnime } from "./utils-for-related-anime-or-manga-show-component";
type RelatedCardForAnimeProps = { shikimori_id: number; relation: string };
export async function RelatedCardForAnime({ shikimori_id, relation }: RelatedCardForAnimeProps): Promise<JSX.Element | null> {
    ...
    return (
        <Link href={"/anime/" + data.shikimori_id} className={relatedCardStyles}>
            <ShowImageForRelatedAnimeSection title={`Обложка от аниме ${data.title}`} url={getAnimePosterUrlByShikimoriId(data.shikimori_id)} />
