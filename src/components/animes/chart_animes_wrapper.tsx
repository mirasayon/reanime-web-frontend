...
export function ChartCarouselWrapper({ animes, render_images = true }: { render_images?: boolean; animes: i_top_charts_anime_json[] }) {
    return (
                    ...
                    <div key={one_slide.page_url}>
                        <Link href={one_slide.page_url} className={"flex w-screen text-sm"}>
                            {render_images ? (
                                ...
                                <div className={"h-[300px] flex justify-center p-8 text-4xl  "}>{slide_index}</div>
                            )}
                            <section className={"m-1 h-[300px] text-wrap p-1 w-[50%] overflow-y-scroll flex flex-col"}>
