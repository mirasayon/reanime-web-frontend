    ...
    return (
        <div className="p-10 flex-row justify-evenly flex ">
            <form onSubmit={onSubmit} className="w-2xl border-4 border-blue-500/50 rounded-lg p-8 flex text-xl flex-col m-4 " noValidate>
                ...
                <InputLoginForAuthForm fieldError={clientErrors.username} props={register("username", { required: true })} />
                <InputPasswordForAuthForm fieldError={clientErrors.password} props={register("password", { required: true })} />
                {serverErrors?.map((msg, i) => (
                    <p key={i} className=" dark:text-red-500 text-red-800">
                        {msg}
                    </p>
                ))}
