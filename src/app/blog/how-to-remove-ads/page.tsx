...
export default function HowToRemoveAds() {
    return (
        <main className={`${BORDER} p-6 max-w-6xl mx-auto bg-white/60 dark:bg-slate-900/50 shadow-sm`}>
            ...
            <p className="text-sm mb-4 dark:text-slate-300">
                Кратко: реклама плеера загружается с внутренних доменов, не с самого плеера. Если блокировать загрузку
                скриптов с этих доменов, реклама не появится — вместо неё могут быть короткие паузы (~2 с), пока
                блокировка срабатывает. Этот способ работает и на телефонах — например, в мобильном Firefox, где
